...
export default class QuestionRepository implements IQuestionRepository {
    ...
    async findByLanguageId(languageId: LanguageId): Promise<Question[]> {
        try {
            const questions = await QuestionModel.find({ languageId: languageId.getValue() });
    ...
    async update(question: Question): Promise<boolean> {
        try {
            ...
            const savedQuestion = await QuestionModel.findOne({id: questionId.getValue()});
            if (!savedQuestion) {
                throw new Error("Question not found");
            }
            const result = await QuestionModel.updateOne({id: questionId.getValue()}, toMongoModel(question));
