...
export default class UpdateQuestionUseCase extends UseCaseBase<Input, boolean> {
    ...
    override async execute(input: Input): Promise<boolean> {
        try {
            ...
            const newQuestions = input.descriptions.map(desc => QuestionDescription.create(desc));
            const question = await this.questionRepository.findById(questionId);
            if (!question) {
                throw new Error("Question not found");
            }

            const languageId = input.languageId ? LanguageId.create(input.languageId) : question.languageId;
