...
export default class GetQuestionsByLanguageIdUseCase extends UseCaseBase<string, Question[]> {
    ...
    override async execute(input: string): Promise<Question[]> {
        try {
            ...
            const questions = await this.questionRepository.findByLanguageId(languageId);
