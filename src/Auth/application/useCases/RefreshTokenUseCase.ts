...
export default class RefreshTokenUseCase extends UseCaseBase<RefreshTokenInput, RefreshTokenOutput> {
    ...
    override async execute(input: RefreshTokenInput): Promise<RefreshTokenOutput> {
        const { userId, token } = input;

        const userStored = await this.authRepository.getCredentialsByUserId(UserId.create(userId));
        if (!userStored) {
        ...
        const tokenRefreshed = await this.authRepository.refreshToken(UserId.create(userId), Token.create(token));
        if (!tokenRefreshed) {
