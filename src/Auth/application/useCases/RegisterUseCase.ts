...
export default class RegisterUseCase extends UseCaseBase<RegisterInput, RegisterOutput> {
    constructor(
    ...
    override async execute(input: RegisterInput): Promise<RegisterOutput> {
        ...
        const userExists = await this.userRepository.existsByEmail(UserEmail.create(email));
        ...
        const userCreated = await this.userRepository.save(User.createWithoutId(name, email));
        if (!userCreated) {
            return {
                success: false,
        ...
        const credentialsSaved = await this.authRepository.saveCredentials(user.id, hashedPassword);
        if (!credentialsSaved) {
            return {
                success: false,
