...
export default class ChangePasswordUseCase extends UseCaseBase<Input, Output> {
    constructor(private readonly authRepository: IAuthRepository, private readonly userRepository: IUserRepository) {super();}
    ...
    override async execute(input: Input): Promise<Output> {
        ...
        const storedPassword = await this.authRepository.getCredentialsByUserId(UserId.create(userId));
        if (!storedPassword) {
            return {
                success: false,
        ...
        const passwordUpdated = await this.authRepository.updatePassword(UserId.create(userId), hashedPassword);
