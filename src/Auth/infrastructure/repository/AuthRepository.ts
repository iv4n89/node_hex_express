...
export default class AuthRepository implements IAuthRepository {
    ...
    async saveCredentials(userId: UserId, password: AuthPassword): Promise<boolean> {
        const authUser = AuthUser.createWithoutId(userId.getValue(), password.getValue());
    ...
    async getToken(userId: UserId, password: AuthPassword): Promise<Token> {
        const authUserModel = await AuthModel.findOne({ userId: userId.getValue(), password: password.getValue() });
        if (!authUserModel) {
    ...
    async updatePassword(userId: UserId, newPassword: AuthPassword): Promise<boolean> {
        const userIdValue = userId.getValue();
        const result = await AuthModel.updateOne({ userId: userIdValue }, { password: newPassword.getValue() });
        return result.modifiedCount === 1;
    }

    async refreshToken(userId: UserId, token: Token): Promise<Token> {
        const userIdValue = userId.getValue();
        const decryptedToken = Token.verify(token.getValue(), {userId: userId.getValue()});
        if (!decryptedToken) {
