...
export default class AuthService implements IAuthService {
    ...
    constructor() {
        ...
        this.registerUseCase = new RegisterUseCase(authRepository, this.userRepository);
        this.refreshTokenUseCase = new RefreshTokenUseCase(authRepository);
        this.changePasswordUseCase = new ChangePasswordUseCase(authRepository, this.userRepository);
    }

    async login(email: string, password: string): Promise<IAuthServiceOutput | null> {
    ...
    async register(name: string, email: string, password: string): Promise<IAuthServiceOutput | null> {
    ...
    async validateToken(token: string): Promise<IUser | null> {
        ...
        const user = await this.userRepository.findById(UserId.create(userIdFromToken));
        if (!user) {
            return null;
        }
    ...
    async refreshToken(userId: string, token: string): Promise<IAuthServiceOutput | null> {
