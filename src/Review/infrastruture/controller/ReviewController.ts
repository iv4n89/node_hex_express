...
export async function getReviewByUserIdAndFinished(
  req: Request,
  res: Response
) {
  try {
    const { userId } = req.params;
    const reviews = await reviewService.findByUserIdAndFinished(userId);
    if (!reviews) {
...
export async function getReviewByQuestionsIdAndUserId(
  req: Request,
  res: Response
) {
  try {
    const { questionsId, userId } = req.body;
    const reviews = await reviewService.findByQuestionsIdAndUserId(
      questionsId,
      userId
    );
    if (!reviews) {
